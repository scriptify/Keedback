...
const { query: unboundQuery, error, handleQueryPromise, requireLogin, connect } = require(`./util.js`);
...
function createServer({
  ...
  const connection = connect({ host, user, password: dbPass, database });
  const query = unboundQuery.bind(null, connection);
  ...
  const publicAppPath = path.join(process.cwd(), directory);
  ...
  app.use(`/`, (req, res, next) => {
    const isLoggedIn = (req.cookies && req.cookies.session && req.cookies.session.UID !== null && req.cookies.session.UID !== undefined);
    if (!isLoggedIn && req.path === `/` && loginProtected)
      res.redirect(`/login`);
    else
      next();
  });
  ...
  console.log(`Keedback server started at ${ip}:${port}.`);
}
